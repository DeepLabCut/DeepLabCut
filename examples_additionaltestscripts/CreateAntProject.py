...
for projectpath in [p1, p2]:
    ...
    if "escaping" in projectpath:
        ...
        PAF_graph.extend(
            [[4, 6], [6, 8], [6, 14], [6, 27], [6, 17], [6, 30], [6, 33], [6, 20]]
        )  # direct leg
    else:
        PAF_graph = [[0, 1], [0, 2], [1, 2], [2, 3], [3, 1]]  # central bodyparts
        PAF_graph.extend(
            [[4, 5], [2, 5], [1, 6], [1, 4], [4, 6], [5, 6], [6, 2], [3, 6]]
        )
        PAF_graph.extend(
            [[7, 8], [2, 9], [1, 9], [1, 7], [7, 9], [8, 9], [8, 2],]
        )
    ...
    deeplabcut.auxiliaryfunctions.attempttomakefolder(
        Path(os.path.join(projectpath, str(trainingsetfolder))), recursive=True
    )
    Data = deeplabcut.generate_training_dataset.trainingsetmanipulation.merge_annotateddatasets(
        cfg, Path(os.path.join(projectpath, trainingsetfolder)), windows2linux=False
    )
    ...
    for shuffle in range(3):
        ...
        while frac < 0.03:
            ...
            print(len(testindex) * 1.0 / (len(trainindex) + len(testindex)))
            frac = len(testindex) * 1.0 / (len(trainindex) + len(testindex))
        ...
        deeplabcut.create_training_dataset(
            config, Shuffles=[shuffle], trainIndexes=trainindex, testIndexes=testindex
        )
