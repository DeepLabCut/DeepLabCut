...
def train_network(
    ...
    Args:
        ...
        saveiters (optional): Only for the TensorFlow engine (for the PyTorch engine see the
        ...
        maxiters (optional): Only for the TensorFlow engine (for the PyTorch engine see the
            ``torch_kwargs``: you can use ``epochs``). This variable is actually set in
            ``pose_config.yaml``. However, you can overwrite it with this hack. Don't use this regularly,
            just if you are too lazy to dig out the ``pose_config.yaml`` file for the corresponding project.
            If ``None``, the value from there is used, otherwise it is overwritten! Defaults to None.
        epochs (optional): Only for the PyTorch engine (equivalent to the `maxiters` parameter for the
        ...
        save_epochs (optional): Only for the PyTorch engine (equivalent to the `saveiters` parameter for the
        ...
        device (str, optional): Only for the PyTorch engine. The device to run the training on (e.g. "cuda:0").
            Defaults to None.
        snapshot_path (str or Path, optional): Only for the PyTorch engine. The path to the pose model snapshot to
            resume training from. Defaults to None.
        detector_path (str or Path, optional): Only for the PyTorch engine. The path to the detector model snapshot to
            resume training from. Defaults to None.
...
def evaluate_network(
    ...
    **torch_kwargs,
    ...
    Args:
        ...
        comparisonbodyparts (str or list, optional): The average error will be computed for those body parts only. The
        ...
        pcutoff (float | list[float] | dict[str, float] | None): Only for the PyTorch engine. For the TensorFlow engine,
            ...
            one value for each bodypart and one value for each unique bodypart (if there are any). If a dict is
                provided,
            the keys should be bodyparts mapping to pcutoff values for each bodypart. Bodyparts that are not defined in
            the dict will have pcutoff set to 0.6. Defaults to None.
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata. You can
            overwrite this by passing the engine as an argument, but this should generally not be done. Defaults to
                None.
...
def return_evaluate_network_data(
    ...
    Args:
        ...
        comparisonbodyparts (list of bodyparts): The average error will be computed for those body parts only (has to
...
def analyze_videos(
    ...
    **torch_kwargs,
    ...
    Args:
        ...
        batchsize (int or None, optional): Currently not supported by the PyTorch engine. Batch size for inference;
...
def create_tracking_dataset(
    ...
    Args:
        config (str): Full path of the config.yaml file.
        videos (list[str]): A list of strings containing the full paths to videos from which to create a tracking
        ...
        TFGPUinference (bool, optional): Only for the TensorFlow engine. Perform inference on GPU with TensorFlow code.
            ...
            Mert Yüksekgönül, Byron Rogers, Matthias Bethge, Mackenzie W. Mathis.
            Source: https://arxiv.org/abs/1909.11229. Defaults to True.
        destfolder (optional): Specifies the destination folder for analysis data. If ``None``, the path of the video is
...
def analyze_time_lapse_frames(
    ...
    Args:
        ...
        trainingsetindex (int, optional): Integer specifying which TrainingsetFraction to use. By default the first
            (note that TrainingFraction is a list in config.yaml).
        gputouse (int, optional): Only for TensorFlow models. For PyTorch models, please use `device`. Natural number
...
def convert_detections2tracklets(
    ...
    Args:
        ...
        destfolder (string, optional): Specifies the destination folder for analysis data (default is the path of the
...
def extract_save_all_maps(
    ...
    Args:
        ...
        trainingsetindex (int, optional): Integer specifying which TrainingsetFraction to use. By default the first
            (note that TrainingFraction is a list in config.yaml). This variable can also be set to "all".
        comparisonbodyparts (list of bodyparts): The average error will be computed for those body parts only (has to be
