...
def build_llms_txt(
    ...
    `knowledge_base_url` is the published `.../knowledge/` directory;
    `version_label` is the one whose `api.jsonl` and `docs.jsonl` sit directly
    under it (the same version this whole build is for).
    """
    lines = [
        f"# {PROJECT_NAME}",
        "",
    ...
    lines += [
        ...
        f"- [Doc pages and sections]({knowledge_base_url}{version_label}/docs.jsonl): "
        "one JSON object per line, `{id, title, url, section, summary, ...}`",
        f"- [API symbols]({knowledge_base_url}{version_label}/api.jsonl): one JSON "
