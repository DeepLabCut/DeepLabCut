...
datafn,metadatafn=deeplabcut.auxiliaryfunctions.GetDataandMetaDataFilenames(trainingsetfolder,trainFraction,shuffle,cfg)
...
outfolder=basepath #'/home/alex/Dropbox/InterestingCode/social_datasets/Results-SDLCpaper'
            ...
            for ind in individuals:
                for ind2 in individuals:
                    if ind!='single' and ind2!='single':
                        source=np.array(Data[ind,p[0],'x'][jj],Data[ind,p[0],'y'][jj])
                        target=np.array(Data[ind2,p[1],'x'][jj],Data[ind2,p[1],'y'][jj])
                        if ind==ind2:
                            ds_within.append(distance(source,target))
                        else:
