...
def get_individuals_and_bodyparts(cfg):
...
def filter_individuals_from_config(cfg, individuals):
    ...
    if "individuals" not in cfg:  # Not a multi-animal project...
        return [""]
    all_indivs = get_individuals_and_bodyparts(cfg)[0]
    if individuals == "all":
        return all_indivs
...
def filter_unwanted_paf_connections(cfg, paf_graph):
    ...
    multi = get_individuals_and_bodyparts(cfg)[2]
...
def validate_paf_graph(cfg, paf_graph):
    multianimalbodyparts = get_individuals_and_bodyparts(cfg)[2]
    connected = set()
...
def convert2_maDLC(config, userfeedback=True, forceindividual=None):
    ...
    individuals, uniquebodyparts, multianimalbodyparts = get_individuals_and_bodyparts(
        cfg
    )
...
def create_default_inference_cfg(cfg):
    # load defaults
...
def verify_inference_cfg(cfg, inferencecfg):
    template = create_default_inference_cfg(cfg)
