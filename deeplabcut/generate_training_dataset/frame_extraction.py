...
def extract_frames(config,mode='automatic',algo='kmeans',crop=False,checkcropping=False, userfeedback=False, Screens=1,scale_w=.8,scale_h=.8):
    ...
    For selecting frames automatically with 'uniform' and cropping the frames based on the `crop` parameters in config.yaml:

    >>> deeplabcut.extract_frames('/analysis/project/reaching-task/config.yaml','automatic',crop=True)

    For selecting frames automatically with 'uniform', and cropping the frames based on the `crop` parameters in config.yaml and checking for cropping:
    ...
    While selecting the frames manually, you do not need to specify the `crop` parameter in the command.
