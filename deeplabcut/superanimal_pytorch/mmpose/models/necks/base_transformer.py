...
class TokenPose_L_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim,
                 apply_init=False, hidden_heatmap_dim=64*6,heatmap_dim=64*48,
                 heatmap_size=[64,48], channels = 3, dropout = 0., emb_dropout = 0.,
        ...
        # self.mlp_head = nn.Sequential(
        #     nn.LayerNorm(dim*3),
        #     nn.Linear(dim*34, hidden_heatmap_dim),
        #     nn.LayerNorm(hidden_heatmap_dim),
        #     nn.Linear(hidden_heatmap_dim, heatmap_dim)
    ...
    def forward(self, feature, mask = None):
        ...
        if isinstance(feature, list):
            # hrnet returns a list, thus adding a extra axis in the leftmost of feature
            feature = feature[0]
