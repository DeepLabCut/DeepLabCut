...
from deeplabcut.pose_estimation_tensorflow.nnet.processing import (
    Predictor,
    Pose,
    TrackingData,
)
...
class FastPlotterArgMax(Predictor):
    ...
    def __init__(
        ...
        (__, self._title_font_height), self._title_baseline = cv2.getTextSize(
            self.TEST_TEXT, self.TITLE_FONT, self.TITLE_FONT_SIZE, self.FONT_THICKNESS
        )
    ...
    def _compute_video_measurements(self, scmap_width: int, scmap_height: int):
        ...
        self._subplot_height = (
            (self.PADDING * 2) + total_subplot_text_height + self._scmap_height
        )
        ...
        view_shape = (
            self.MULTIPLIER,
            self.MULTIPLIER,
            shape[0] // self.MULTIPLIER,
            shape[1] // self.MULTIPLIER,
            shape[2],
        )
        view_strides = (
            strides[0],
            strides[1],
            strides[0] * self.MULTIPLIER,
            strides[1] * self.MULTIPLIER,
            strides[2],
        )
    ...
    def _draw_title(self, text: str):
        ...
        (width, __), __ = cv2.getTextSize(
            text, self.TITLE_FONT, self.TITLE_FONT_SIZE, self.FONT_THICKNESS
        )
    ...
    def _draw_subplot(
        ...
        grayscale_img = self._probs_to_grayscale(
            self._logify(prob_map) if (self.LOG_SCALE) else prob_map
        )
        self._colormap_view[:, :] = cv2.applyColorMap(
            grayscale_img, self.COLORMAP, self._unscaled_cmap_temp
        )
        # Insert the probability map...
        subplot_top_x, subplot_top_y = (
            (x_upper_corner + self.PADDING) - 1,
            (y_upper_corner + self.PADDING) - 1,
        )
        ...
        (text_width, __), __ = cv2.getTextSize(
            bp_name, self.SUBPLOT_FONT, self.SUBPLOT_FONT_SIZE, self.FONT_THICKNESS
        )
    ...
    def get_settings(cls) -> Union[List[Tuple[str, str, Any]], None]:
        ...
        return [
            ...
            (
                "subplot_font_size",
                "Float, the font size of the titles of each subplot.",
                1.5,
            ),
            (
                "subplot_font",
