...
class PoseNet:
    ...
    def prediction_layers(self, features, end_points, input_shape, reuse=None, is_training=False, scope='pose'):
        cfg = self.cfg

        num_layers = re.findall("resnet_([0-9]*)", cfg.net_type)[0]
        layer_name = 'resnet_v1_{}'.format(num_layers) + '/block{}/unit_{}/bottleneck_v1'
        final_dims = tf.ceil(tf.divide(input_shape[1:3], tf.convert_to_tensor(16))) #of the RESNET!
        ...
        with slim.arg_scope([slim.conv2d_transpose], padding='SAME',
            ...
            with tf.variable_scope('upsampled_features'):
                upsampled_features = slim.conv2d_transpose(features, cfg.bank5,
                                                  kernel_size=[3, 3], stride=2,
                                                  scope='block4')
        ...
        with tf.variable_scope(scope, reuse=reuse):
            out['part_pred'] = prediction_layer(cfg, net, 'part_pred',
            ...
            if cfg.pairwise_predict and "multi-animal" not in cfg.dataset_type:
                    out['pairwise_pred'] = prediction_layer(cfg, net, 'pairwise_pred',
