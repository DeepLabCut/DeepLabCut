...
class FrameExporter(Predictor):
    ...
    def __init__(
        ...
        self.COMPRESSION_LEVEL = (
            ...
            if (0 <= settings["compression_level"] <= 9)
            else 6
        )
    ...
    def get_settings(cls) -> Union[List[Tuple[str, str, Any]], None]:
        return [
            (
                "sparsify",
                "Boolean, specify whether optimize and store the data in a sparse format when it "
                "saves storage",
                True,
            ),
            (
                "threshold",
                "A Float between 0 and 1. The threshold used if sparsify is true. Any values which land below "
