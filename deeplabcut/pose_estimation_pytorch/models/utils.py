...
def generate_heatmaps(
    ...
    if heatmap_type == "gaussian":
        scmap, weights, locref_map, locref_mask = gaussian_scmap(
            cfg, coords, scale_factor, heatmap_size
        )
    elif heatmap_type == "plateau":
        scmap, weights, locref_map, locref_mask = plateau_scmap(
            cfg, coords, scale_factor, heatmap_size
        )
    else:
...
def gaussian_scmap(cfg, coords, scale_factors, heatmap_size):
    ...
    scmap = np.zeros((heatmap_size[0], heatmap_size[1], num_joints), dtype=np.float32)

    locref_map = np.zeros(
        (heatmap_size[0], heatmap_size[1], num_joints * 2), dtype=np.float32
    )
...
def _generate_heatmaps(keypoints, heatmap_size, image_size=(256, 256), sigma=5):
    """
    TODO: MAKE FASTER
    Parameters
    ----------
    ...
    """
    target = torch.zeros(
        (keypoints.shape[0], heatmap_size[1], heatmap_size[0]), dtype=torch.float32
    )
