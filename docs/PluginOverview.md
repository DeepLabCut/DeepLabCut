...
DeepLabCut currently comes with 4 "default" plugins:
 ...
 - "frame_exporter": Makes predictions in the same exact way as "argmax",
   but also exports the probability frames generated by the neural network
   to a custom binary file format (DeepLabCut Frame Store, ".dlcf"). Predictors
   can be rerun on the frames stored in the DLCF using the "analyze_frame_store"
   method.
...
class PrintAndPredict(Predictor):
    """
    A simple example plugin which prints frames to the command line... (this will be used as the plugin's description).
    """
