...
#from .inference_network import DLCTransReID
...
class DLCTransReID(nn.Module):
    def __init__(self,  in_chans=2048, num_kpts = 12, embed_dim=768, depth=4,
                 num_heads=4, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., camera=0, view=0,
...
def dlc_base_kpt_TransReID(drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, local_feature=False,sie_xishu=1.5, kpt_num = 12, **kwargs):
    ...
    mlp_ratio = 1
    num_kpts = kpt_num 
    model = DLCTransReID(embed_dim=embed_dim, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=True,
                         drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, num_kpts = num_kpts,
...
def load_features_from_coord(features, key, coords, valid_mask_for_fish = False):
...
class DLCTrans:
    ...
    def _load_params(self):
        self.model.load_param(self.checkpoint)

    def _get_vec(self,inp_a, inp_b):
        coord_a, frame_a = inp_a
        coord_b, frame_b = inp_b
    ...
    def __call__(self,inp_a,inp_b, return_features = False):
        ...
        _tuple = self._get_vec(inp_a,inp_b)
        if _tuple is None:
            return None
        vec_a, vec_b = _tuple
...
if __name__ == '__main__':
    ...
    path_to_features =  os.path.join(data_folder,'deeplc.menidia.feeding.school3.176pm.S04.D.shortDLC_resnet50_SchoolingMay7shuffle1_200000_features.pickle')

    h5file = os.path.join(data_folder,'fishgt.h5')
    df_gt = pd.read_hdf(h5file)
    
    with open(path_to_features, 'rb') as f:
        features = pickle.load(f)    

    tracks = []
    for i, group in df_gt.groupby('individuals', axis=1):
        inds = group.index.to_numpy()
        tracks.append(Tracklet(group.to_numpy().reshape((len(group), -1, 3)), inds))
    ts = TrackletStitcher(tracks, n_tracks=3, split_tracklets=False)
    triplets = ts.mine(1)
    ...
    dlctrans = DLCTrans(checkpoint = 'fish_dlc_transreid_140.pth',
                        kpts_num = 5, feature_path = feature_path)

    dist_ap = dlctrans(anchor,pos)
    dist_an = dlctrans(anchor,neg)

    print ('ap',dist_ap)
    print ('an',dist_an)    
    # test by reading some features
