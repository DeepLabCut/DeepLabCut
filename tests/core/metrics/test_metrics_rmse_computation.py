...
def test_detection_rmse(
    gt: list,
    pred: list,
    data_unique: tuple[list, list] | None,
    result: tuple[float, float],
):
    data = [(np.asarray(gt), np.asarray(pred))]
    data_unique = (
        [(np.asarray(data_unique[0]), np.asarray(data_unique[1]))]
        if data_unique
        else None
    )
    expected_rmse, expected_rmse_cutoff = result
    rmse, rmse_cutoff = compute_detection_rmse(
        data, pcutoff=0.6, data_unique=data_unique
    )
    assert_almost_equal(rmse, expected_rmse)
    assert_almost_equal(rmse_cutoff, expected_rmse_cutoff)
