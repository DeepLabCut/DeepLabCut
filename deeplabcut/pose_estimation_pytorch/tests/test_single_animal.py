...
from deeplabcut.pose_estimation_pytorch.solvers.utils import get_paths, get_results_filename
...
def get_training_set_length(cfg, train_fraction, shuffle):
    ...
    training_folder = os.path.join(cfg["project_path"], deeplabcut.auxiliaryfunctions.get_training_set_folder(cfg))
    train_idx_path = os.path.join(training_folder, f'Documentation_data-{cfg["Task"]}_{int(train_fraction*100)}shuffle{shuffle}.pickle')

    with open(train_idx_path, "rb") as file:
        meta = pickle.load(file)

    print(f'length of the training set {len(meta[1])}, length of the test set {len(meta[2])}')
    return len(meta[1])

def load_model(cfg, pytorch_config, shuffle, model_prefix="", train_iteration=-1):
    names = get_paths(train_fraction=cfg['TrainingFraction'][0],
    ...
    pose_cfg = deeplabcut.auxiliaryfunctions.read_config(pytorch_config['pose_cfg_path'])
    model = build_pose_model(pytorch_config['model'], pose_cfg)
    model.load_state_dict(torch.load(names['model_path']))

    return model

def evaluate_network_custom(config_path, shuffle, model_prefix="", transform=None, train_iteration=-1):
    ...
    project = deeplabcut.pose_estimation_pytorch.DLCProject(shuffle=shuffle,
                             proj_root=pytorch_config['project_root'])

    valid_dataset = deeplabcut.pose_estimation_pytorch.PoseDataset(project,
                                    transform=transform,
                                    mode='train')
...
def runBenchmark(path_dataset, train_fraction, shuffle, transform = None):
    ...
    deeplabcut.pose_estimation_pytorch.apis.train.train_network(config_path, shuffle=shuffle, transform=transform)
