...
@pytest.mark.parametrize(
    "fake_project, algo, crop",
    [("project_single", "kmeans", False), ("project_multi", "uniform", True)],
...
def test_extract_frames(fake_project, algo, crop):
    ...
    frame_extraction.extract_frames(
        config_path, algo=algo, crop=crop, userfeedback=False
    )
...
def test_create_training_dataset(mocker, project_single, network, aug):
    ...
    columns = pd.MultiIndex.from_product(
        ([conftest.SCORER], bodyparts, ["x", "y"]),
        names=["scorer", "bodyparts", "coords"],
    )
    index = [
        os.path.join(rel_folder, image)
        for image in grab_files_in_folder(image_folder, "png")
    ]
    fake_data = np.tile(
        np.repeat(50 * np.arange(len(bodyparts)) + 100, 2), (len(index), 1)
    )
    ...
    fracs = [0.4, 0.6, 0.8]
    train_inds, test_inds = zip(
        *[
            trainingsetmanipulation.SplitTrials(range(conftest.NUM_FRAMES), frac)
            for frac in fracs
        ]
    )
    shuffles = [1] * len(fracs)
    mocker.patch.object(
        auxfun_models, "Downloadweights", return_value=None
    )  # Avoid downloading weights
    splits = trainingsetmanipulation.create_training_dataset(
        config_path,
        Shuffles=shuffles,
        trainIndexes=train_inds,
        testIndexes=test_inds,
        net_type=network,
        augmenter_type=aug,
    )
...
@pytest.mark.parametrize(
    "fake_project, scale, draw_skeleton, visualizeindividuals",
    [("project_single", 1, True, False), ("project_multi", 0.5, False, True)],
    indirect=["fake_project"],
)
