...
def topdown_video_inference(
    ...
    env = dict(os.environ)
    env['PYTHONPATH'] =  os.pathsep.join([os.path.join(root, "third_party"), env['PYTHONPATH']])
    ...
    inference_demo = os.path.join(root, "third_party", "demo", "top_down_video_demo_with_mmdet.py")
    ...
    inference_only_command = [
        ...
        "--out-video-root", video_outdir,
        "--kpt-thr", f"{kpt_threshold}" ,
        "--dataset-info", f"{dataset_info_path}"
    ]
    ...
    if video_adapt == False:
        ...
        pseudo_label_command = [
            "python", convert_code,
            "--video_result_path", result_file,
            "--video_path", video_path
        ]

        pose_train_code = os.path.join(root, "third_party", "tools", "train.py")
        train_ann_file = os.path.join(video_outdir, f"annotation_{video_name}", "annotations","train.json")
        val_ann_file = os.path.join(video_outdir, f"annotation_{video_name}", "annotations", "test.json")
        img_prefix = os.path.join(video_outdir, f"annotation_{video_name}", "images", "")
        adaptation_weight_folder = os.path.join(video_outdir, f"{video_name}_adapted")


        adaptation_command = [
            "python", pose_train_code, pose_config,
            "--cfg-options",
            ...
            "--work-dir", adaptation_weight_folder
        ]
        ...
        inference_adapted_model_command = [
            ...
            "--video-path", video_path,
            "--out-video-root", f"video_pred_{video_name}",
            "--kpt-thr", f"{kpt_threshold}" ,
            "--kpt-median-filter",
            "--dataset-info", f"{dataset_info_path}"            
        ]        
