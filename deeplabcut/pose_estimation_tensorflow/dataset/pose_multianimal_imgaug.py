...
class MAPoseDataset:
    ...
    def build_augmentation_pipeline(self,
        ...
        if height is not None and width is not None:
            pipeline.add(iaa.Sometimes(cfg.cropratio,iaa.CropAndPad(percent=(-0.3, 0.1),keep_size=False)))
    ...
    def get_batch(self):
        ...
        for i in range(self.batch_size):
            ...
            if self.has_gt:
                ...
                joint_id = [Joints[person_id][:, 0].astype(int) for person_id in Joints.keys()]
                joint_points = np.concatenate([Joints[person_id][:, 1:3] for person_id in Joints.keys()])
    ...
    def get_targetmaps_update(self, joint_ids, joints, data_items, sm_size, target_size):
        ...
        for i in range(len(data_items)):
            ...
            if self.cfg.get("scmap_type", None) == "gaussian":
                ...
                part_score_target, part_score_weight, locref_target, locref_mask, partaffinityfield_target, partaffinityfield_mask = self.compute_target_part_scoremap_numpy(
                    joint_ids[i], [joints[i]], data_items[i], sm_size, scale)
    ...
    def next_batch(self,plotting=False):
        while True:
            ...
            if self.has_gt:
                targetmaps = self.get_targetmaps_update(joint_ids, batch_joints, data_items, sm_size, image_shape)
                ...
                if len(joint_ids)>1: #otherwise there cannot be a joint!
                    ...
                    for l in range(self.cfg.num_limbs):
                        ...
                        if (len(I1)>0)*(len(I2)>0):
                            ...
                            if dist>0:
                                Dx=(linkedj_x-j_x)*1./dist #x-axis UNIT VECTOR
                                Dy=(linkedj_y-j_y)*1./dist

                                d1=[np.asscalar(Dx*j_x+Dy*j_y),np.asscalar(Dx*linkedj_x+Dy*linkedj_y)] #in-line with direct axis
                                d1lowerboundary=min(d1)
                                d1upperboundary=max(d1)
                                d2mid=np.asscalar(j_y*Dx-j_x*Dy) #orthogonal direction

                                distance_along=Dx*(x*stride+half_stride)+Dy*(y*stride+half_stride)
                                distance_across=(((y*stride+half_stride)*Dx-(x*stride+half_stride)*Dy)-d2mid)*1./self.cfg.pafwidth*scale
                                ...
                                if self.cfg.weigh_only_present_joints:
                                    partaffinityfield_mask[mask, l*2+0] = 1.
                                    partaffinityfield_mask[mask, l*2+1] = 1.
