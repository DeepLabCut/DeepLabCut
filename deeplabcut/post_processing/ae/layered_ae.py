...
def map_pred3(sample, output_size, batch_size):
    return (sample, tf.convert_to_tensor(np.zeros((batch_size, output_size), dtype=np.float32)),
            tf.convert_to_tensor(np.zeros((batch_size, output_size * 3), dtype=np.float32)))
...
class AE:
    def __init__(self, network_arch, batch_size, train_epochs, model_path=dirname + "/training_output/ae_model",
        ...
        predictingData = predictingData.map(lambda x: map_pred3(x, self.output_size, self.batch_size))
        ...
        self.x_reconstr_mean_one = tf.add(self.x_residuals,
                                          x_in[:, self.output_size:self.output_size * (int(self.time_pair / 2) + 2)])
        self.x_reconstr_mean_one = transfer_fct(self.x_reconstr_mean_one)
        self.x_reconstr_normalized = tf.layers.batch_normalization(self.x_reconstr_mean_one, axis=1,
        ...
        self.x_residuals_two = tf.contrib.layers.fully_connected(dlayer_5, output_size, activation_fn=None,
                                                                 weights_initializer=init,
                                                                 biases_initializer=tf.zeros_initializer())
        self.x_reconstr_mean_two = tf.add(self.x_residuals_two,
                                          self.x_reconstr_mean_one[:, self.output_size:self.output_size * 2])
        ...
        self.cost = self.proportion_intermediate * tf.reduce_mean(
            reconstr_loss_one) + self.proportion_end * tf.reduce_mean(reconstr_loss_two)  # average over batch
        ...
        self.train_writer = tf.summary.FileWriter(dirname + "/training_output/" + model_summary)
    ...
    def train(self, datax, datainter, datay, inter_prop=0.5, end_prop=0.5, move_epoch=None, save_epoch=None):
        self.sess.run(self.training_init,
                      feed_dict={self.training_x: datax, self.training_z: datainter, self.training_y: datay})
        if not move_epoch:
            move_iter = int((len(datax) / self.batch_size) * (self.train_epochs / 2))
        else:
            move_iter = int((len(datax) / self.batch_size) * move_epoch)
        if not save_epoch:
            save_iter = int((len(datax) / self.batch_size) * (self.train_epochs / 10))
        else:
            save_iter = int((len(datax) / self.batch_size) * save_epoch)
    ...
    def reconstruct(self, x):
        ...
        while True:
            try:
                nparray = np.append(nparray,
                                    self.sess.run(self.x_reconstr_mean_two, feed_dict={self.is_training: False}),
                                    axis=0)
            except tf.errors.OutOfRangeError:
                break
        return nparray
