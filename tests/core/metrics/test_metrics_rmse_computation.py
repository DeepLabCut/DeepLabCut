...
def test_rmse_single_image(gt: list, pred: list, result: tuple[float, float]):
    ...
    rmse, rmse_cutoff = compute_rmse(data, pcutoff=0.6)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [  # ground truth pose
                [[10.0, 10.0, 2], [float('nan'), float('nan'), 0]],
                [[float('nan'), float('nan'), 0], [20.0, 20.0, 2]],
            ],
            [  # predicted pose
                [[20.0, 20.0, 0.9], [21.0, 20.0, 0.9]],  # swapped prediction order
                [[15.0, 10.0, 0.4], [10.0, 10.0, 0.9]],
            ],
